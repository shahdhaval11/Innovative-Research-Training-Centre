...
// ─── Services Section (merged selector + detail) ──────────────────────────────
function ServicesSection() {
  ...
  const handleSelect = (i: number) => {
    setActiveIndex(i);
    setOpenItem(null); // reset accordion on service switch
  };

  return (
    <section id="services" className="bg-[#f7f9fb] py-20 px-6">
      <div className="max-w-7xl mx-auto">

        {/* ── Centered header ── */}
        <div className="text-center mb-14">
          <span className="text-[#F4A261] font-bold text-sm uppercase tracking-widest">All Service Areas</span>
          <h2 className="text-4xl font-black text-[#003049] mt-2">Complete Support Ecosystem</h2>
          <p className="text-[#003049]/60 mt-3 max-w-2xl mx-auto text-base">
            Seven interconnected service pillars designed to cover every dimension of your academic
            research journey. Select a service to explore its full details.
          </p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          ...
          {/* ── Left sidebar — service list (desktop only) ── */}
          <div className="hidden lg:flex flex-col gap-2 w-72 shrink-0">
            {SERVICES.map((s, i) => {
              ...
              return (
                <button
                  ...
                  className={`group w-full text-left flex items-center gap-4 px-4 py-4 rounded-2xl border transition-all duration-200 ${
                    isActive
                      ? "bg-[#003049] border-[#003049] shadow-lg"
                      : "bg-white border-[#003049]/8 hover:border-[#F4A261]/50 hover:shadow-md"
                  }`}
                >
                  {/* Icon */}
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl shrink-0 transition-all duration-200 ${
                    isActive
                      ? "bg-[#F4A261]"
                      : "bg-[#003049]/6 group-hover:bg-[#F4A261]/15"
                  }`}>
                    {s.icon}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-xs font-bold uppercase tracking-widest mb-0.5 transition-colors ${
                      isActive ? "text-[#F4A261]" : "text-[#003049]/40"
                    }`}>
                      {String(i + 1).padStart(2, "0")}
                    </p>
                    <p className={`text-sm font-bold leading-tight truncate transition-colors ${
                      isActive ? "text-white" : "text-[#003049] group-hover:text-[#003049]"
                    }`}>
                      {s.title}
                    </p>
                    <p className={`text-xs mt-0.5 transition-colors ${
                      isActive ? "text-white/50" : "text-[#003049]/40"
                    }`}>
                      {s.items.length} offerings
                    </p>
                  </div>
            ...
            {/* CTA card in sidebar */}
            <div className="mt-2 bg-[#F4A261] rounded-2xl p-5">
          ...
          <div className="lg:hidden w-full mb-6 overflow-x-auto pb-2 flex gap-3 snap-x">
            {SERVICES.map((s, i) => {
              const isActive = i === activeIndex;
              return (
                <button
                  key={s.id}
                  onClick={() => handleSelect(i)}
                  className={`snap-start shrink-0 flex items-center gap-2 px-4 py-2.5 rounded-full border text-sm font-semibold transition-all duration-200 ${
                    isActive
                      ? "bg-[#003049] border-[#003049] text-white"
                      : "bg-white border-[#003049]/15 text-[#003049]/70 hover:border-[#003049]/40"
                  }`}
                >
                  <span>{s.icon}</span>
                  <span>{s.title}</span>
          ...
          {/* ── Right panel — active service detail ── */}
          <div className="w-full lg:flex-1 lg:min-w-0">
            <div className="bg-white rounded-3xl border border-[#003049]/8 shadow-sm overflow-hidden">

              {/* Detail header */}
              <div className="bg-[#003049] px-8 py-7 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-[#F4A261]/20 border border-[#F4A261]/30 flex items-center justify-center text-3xl shrink-0">
                    {active.icon}
                  </div>
                  <div>
                    <span className="text-[#F4A261] text-xs font-bold uppercase tracking-widest">
                    ...
                    <h2 className="text-2xl font-black text-white mt-0.5 leading-tight">
                      {active.title}
                    </h2>
                    <p className="text-white/50 text-sm mt-0.5 italic">{active.tagline}</p>
                  </div>
                </div>
                <a
                  href="/contactus"
                  className="shrink-0 bg-[#F4A261] text-[#003049] text-sm font-black px-6 py-2.5 rounded-full hover:bg-[#E76F51] transition-colors duration-200"
                >
                  Get Started →
                </a>
              </div>
              ...
              <div className="p-8">
                <div className="grid lg:grid-cols-3 gap-8 items-start">
                  ...
                  <div className="lg:col-span-1">
                    <div className="bg-[#f7f9fb] border border-[#003049]/8 rounded-2xl p-6">
                      <p className="text-[#003049]/75 text-sm leading-relaxed mb-6">{active.desc}</p>
                      <div className="border-t border-[#003049]/8 pt-5">
                        <p className="text-[#F4A261] text-xs font-bold uppercase tracking-widest mb-3">
                          What&apos;s Included
                        </p>
                        <div className="flex flex-col gap-2">
                          {active.items.slice(0, 4).map((item) => (
                            <div key={item.name} className="flex items-center gap-2">
                              <div className="w-1.5 h-1.5 rounded-full bg-[#F4A261] shrink-0" />
                              <span className="text-[#003049]/70 text-xs">{item.name}</span>
                            </div>
                          ))}
                          {active.items.length > 4 && (
                            <p className="text-[#003049]/40 text-xs mt-1">
                              + {active.items.length - 4} more offerings
                            </p>
                          )}
                        </div>
                      </div>
                    ...
                    {active.items.map((item, i) => (
                      <div
                        ...
                        className={`border rounded-2xl overflow-hidden transition-all duration-300 ${
                          openItem === i
                            ? "border-[#F4A261] shadow-md"
                            : "border-[#003049]/10 hover:border-[#003049]/25"
                        }`}
                      >
                        <button
                          onClick={() => setOpenItem(openItem === i ? null : i)}
                        ...
                        >
                          ...
                          <span className={`text-lg font-black transition-all duration-300 shrink-0 ml-3 ${
                            openItem === i ? "text-[#F4A261] rotate-45" : "text-[#003049]/30"
                          }`}>
                            +
                          </span>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {PROCESS.map((p, i) => (
            <div
              key={p.step}
              className="relative bg-[#f7f9fb] border border-[#003049]/8 rounded-2xl p-7 hover:shadow-lg hover:border-[#F4A261]/40 transition-all duration-300 group"
            >
              {i < PROCESS.length - 1 && (
                <div className="hidden lg:block absolute top-10 -right-3 w-6 h-0.5 bg-[#F4A261]/40 z-10" />
              )}
